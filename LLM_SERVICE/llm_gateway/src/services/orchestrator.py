...
class LLMGraphOrchestrator:
    ...
    async def health_check(self) -> HealthCheck:
        try:
            llm_healthy, retriever_healthy, question_builder_healthy, context_builder_healthy, system_prompt_builder_healthy = await asyncio.gather(
                ...
                self.system_prompt_builder_adapter.health_check()
            )

            return HealthCheck(
                overall_status=ServiceStatus.HEALTHY if llm_healthy and retriever_healthy and question_builder_healthy and context_builder_healthy and system_prompt_builder_healthy else ServiceStatus.UNHEALTHY,
                ...
                embedder_status=ServiceStatus.HEALTHY if question_builder_healthy and context_builder_healthy and system_prompt_builder_healthy else ServiceStatus.UNHEALTHY
            )

...
class GrpcTools:
    ...
    def qdrant_normalize(value: Any) -> Any:
        if isinstance(value, dict):
            return {k: GrpcTools.qdrant_normalize(v) for k, v in value.items()}

        if isinstance(value, list):
            return [GrpcTools.qdrant_normalize(item) for item in value]

        if isinstance(value, tuple):
            return tuple(GrpcTools.qdrant_normalize(item) for item in value)
        ...
        if hasattr(value, '__dict__'):
            return {
                key: GrpcTools.qdrant_normalize(val)
        ...
        def decorator(func: Callable[P, Awaitable[R]]) -> Callable[P, Awaitable[R]]:
            ...
            async def wrapper(*args: P.args, **kwargs: P.kwargs) -> R:
                ...
                try:
                    ...
                    logger.warning(
                        f"gRPC request timeout: {method_name} from {peer} "

...
class GrpcBudgetEstimatorServer:
    def __init__(self) -> None:
        self.stub = policy_builder_pb2_grpc
        self._server_addr: str = (
            f"{EnvTools.get_service_host('budget_estimator')}:{EnvTools.get_service_grpc_port('budget_estimator')}"
        )
    ...
    async def start(self) -> None:
        ...
        logger.info(
            f"{colorama.Fore.GREEN}gRPC budget_estimator started at "
    ...
    async def stop(self, grace: float = 5.0) -> None:
        ...
        logger.info(f"{colorama.Fore.YELLOW}Stopping gRPC budget_estimator{colorama.Style.RESET_ALL}")
        await self._grpc_server.stop(grace=grace)
        self._grpc_server = None
        logger.info(f"{colorama.Fore.GREEN}gRPC budget_estimator stopped{colorama.Style.RESET_ALL}")
